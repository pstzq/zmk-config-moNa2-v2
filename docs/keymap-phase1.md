...
2026-08-12 のビルド成功時点で、**ZMK 本体を含む全20プロジェクトを SHA へ pin 済み**。

旧構成の `zmk` は `v0.3-branch+dya`（リリース線）だったためブランチ参照でも比較的安全だったが、**`main+dya` は開発本流**であり、何もしていないのにビルドが壊れうる。そのため今回は ZMK 本体も pin した。

> **更新時の注意**: `config/west.yml` の `zmk` の SHA と `.github/workflows/build.yml` の `uses:` の SHA は**必ず同時に**更新すること。片方だけ動かすとビルドワークフローと ZMK 本体の版が食い違う。

更新したい場合は `git ls-remote <リポジトリURL> <ブランチ>` で新しい SHA を確認し、ビルドが通ることを確認してから pin を進める。
