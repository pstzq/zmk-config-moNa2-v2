...
- **JIS の Shift 面を「対応する基底キーの裏」に配置**して直感性を重視：
  - `@` の裏＝`` ` ``、`ー(−)` の裏＝`=`、`,` の裏＝`<`、`.` の裏＝`>`、`/` の裏＝`?`。
- 上段: `! " # $ %` / `& ' ( ) |`（数字段の Shift 面）。`|` は P 位置（0 の裏）に1つだけ。
- 中段: `+ - _ \` ＋ 中央 `` ` `` / `[ ] { }` ＋ `=`（ー の裏）。
- 下段: `^ ~`（Z/X）＋中央 `*` / `; :`（N/M, **左がセミコロン**）＋ `< > ?`。最下段に `¥`。
- `/` `'` はベース層／上段に出るため SYM の重複は撤去。
...
- 右手下段にメディアキー（両 NAV 共通）: `N/M/,/./ /` 位置 ＝ `⏮ ⏯ ⏭ 🔉 🔊`（`C_PREV / C_PP / C_NEXT / C_VOL_DN / C_VOL_UP`）。BLE 層から移設。
