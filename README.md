...
| `F` 長押し中（400ms） | **精密モード**（カーソルが 1/3 速 + 右手クリック） |
...
`F`（左手人差し指）を **400ms 長押し**すると精密モードに入り、トラックボールのカーソル速度が **1/3** になります。細かい位置合わせ・小さいUI要素のクリック・ドラッグ用です。左手で `F` を押さえたまま、右手でボールを操作しつつクリックできます。
...
タップ（短押し）は通常どおり `F` が入力されます。長押し判定は `tap-preferred` 方式なので、**他のキーを押しても精密モードには落ちません**（時間経過でのみ発動）。`D`+`F` の Shift+Tab コンボ（timeout 50ms）も従来どおり先に成立します。

> 長押し時間は `config/mona2.keymap` の `prec_ht` の `tapping-term-ms`、カーソル速度は `boards/shields/mona2/mona2_r.overlay` の `zip_xy_scaler 1 3` で調整できます。
