...
| 14 | **NAGINATA**（薙刀式・遊び用） | `B`+`N` 同時押しでトグル |
...
日本語入力法「[薙刀式](https://github.com/eswai/zmk-naginata)」を試せる**トグル層**です（普段使いではなく実験用）。薙刀式は**ローマ字を吐いて OS 側 IME に変換させる**方式なので、**OS の IME をローマ字入力モード**にしておく必要があります（Mac 標準 IME / Google 日本語入力どちらも可）。

| 操作 | 動作 |
|---|---|
| `B` + `N` 同時押し | **薙刀 ON**（かな IME ON ＋ 薙刀層へ） |
| `英数` + `かな` 同時押し（薙刀層中） | **薙刀 OFF**（英数 IME OFF ＋ MAC 層へ戻る） |

- **`Space` と `Enter` がシフトキー**です（薙刀式の同時押しに使用）。この層では `Space` 長押し=SYM は無効になります。
- かな入力の同時押し判定はモジュール（`&ng`）が内部で処理します。ZMK コンボではありません。
- 記号・数字はこの層には持たせていません（`英数`+`かな` で一旦抜けて通常の記号層を使う運用）。
- 既定 OS は macOS。実行時に `F15`(Win) / `F16`(Mac) / `F17`(Linux) で切替可能。

> ⚠️ このモジュールは ZMK 公式 main 前提のため、moNa2 の ZMK フォーク（v0.3系）でのビルド互換は要確認です。
